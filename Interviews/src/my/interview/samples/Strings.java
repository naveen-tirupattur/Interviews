...
public class Strings {
	...
	/*
	 * Implement a regex matcher fnmatch(p, s) --> bool
	 *
	 * p: pattern (GLOB Expression) s: string
	 *
	 * ?: 1 any character *: 0-n any character
	 *
	 *
	 * p: /dev/sd? s: /dev/sda p: t*.py s: test.py
	 */
	...
	public static boolean isMatching(String pattern, int patternIndex, String searchString, int sStringIndex) {
		...
		// If you hit the special character *, increment search index till you find
		// the next character in pattern. if not return false
		if (pattern.charAt(patternIndex) == '*') {
			for (int i = sStringIndex; i < searchString.length(); i++) {
				if (isMatching(pattern, patternIndex + 1, searchString, sStringIndex + i))
					return true;
			}
			return false;
		}
	...
	public static void main(String[] args) {
		...
		// System.out.println("Reverse String: "+reverseStringRecursion("this is a
		// test"));

		// System.out.println("longest common substring:
		// "+longestCommonSubString("ADBABC","BAB"));
		...
		// String[] s = new
		// String[]{"fuckyou","fucme","fuckher","fuckoff","fckthatbitch"};
	...
	public static void reverseString(String s) {
		...
		while (end < s.length()) {
			if (reversedString.charAt(end) != ' ') {
				...
				while (end < s.length() && reversedString.charAt(end) != ' ')
					end++;
