...
public class BacktrackingProblems {
	...
	public static void printPermutations(int[] a, int k, int[] s) {
		...
		for (int i = 0; i < c.size(); i++) {
			...
			// if you have seen similar element in this position before then don't go
			// down this path
			a[k] = c.get(i);
	...
	// Given a position (x,y) find probability that a knight is still in the board
	// after k moves
	...
	public static boolean isValidPosition(int x, int y, Board b) {
		...
		if (x < 0 || x >= b.dimension || y < 0 || y >= b.dimension)
			return false;
		return true;
	}
	...
	public static Board initSudokuBoard() {
		int[][] elements = { { 3, 0, 6, 5, 0, 8, 4, 0, 0 }, { 5, 2, 0, 0, 0, 0, 0, 0, 0 }, { 0, 8, 7, 0, 0, 0, 0, 3, 1 },
	...
	// Check if there is queen in the diagonal to the left of current column
	// (since we are filling column by column)
	...
	public static List<Integer> findCandidates(int[] a, int k, Board b) {
		...
		for (int i = 1; i < b.dimension + 1; i++) {
			if (isValidPosition(p.x, p.y, i, b))
				c.add(i);
		}
