...
public class BitOperations {
	...
	public static void printClosestInteger(int number) {
		...
		int i = 0;
		while (number > 0) {
			// Find the first least significant bit which differs from its immediate
			// neighbor.
	...
	public static int swap(int number, int i, int j) {
		...
		if ((number >> i & 1) != (number >> j & 1)) {
			...
			// use xor to change the bits at indexes. If the bit was 1 at index i, it
			// would be come 0 and vice versa with XOR
			number = number ^ mask;
		}
	...
	public static int findMissingNumber(ArrayList<BitInteger> a, int column) {
		...
		// Go through elements of array and add them to appropriate array based on
		// bit in column
		for (BitInteger i : a) {
		...
		// if count(lsb=0) >= count(lsb=1) then odd number is missing else even
		// number is missing
		if (zeroBits.size() <= oneBits.size()) {
			// check the next column
			int v = findMissingNumber(zeroBits, column + 1);
			// when recursion comes back the MSB would have been updated, so shift the
			// number to left to accommodate LSB
			return (v << 1) | 0;
		} else {
