...
public class DynamicProgrammingProblems {
	...
	public static enum type {
		copy, substitute, insert, delete, invalid
	};
	...
	public static void integerPartitioning(int[] a, int k) {
		...
		// C[n][k] = Minimum possible value of M in all possible combinations of n
		// elements in k partitions
		int[][] c = new int[a.length + 1][k + 1];
		// d[n][k] = Number of elements in k-1 partitions, based on this we know
		// there will be n-d[n][k] elements in last partition
	...
	// public static void reconstructPartition(int[] a, int start, int end, int k,
	// int[][] d) {
	// if(k==1) { }
	// else
	//
	...
	public static int editDistance(String s, String p) {
		...
		for (int i = 0; i < s.length() + 1; i++) {
			for (int j = 0; j < p.length() + 1; j++) {
				...
				operations[0].cost = steps[i - 1][j].cost + 1;
				operations[0].name = "insert";
				operations[1].cost = steps[i][j - 1].cost + 1;
				operations[1].name = "delete";
				operations[2].cost = steps[i - 1][j - 1].cost + 1;
				operations[2].name = "substitute";
				...
				if (a.charAt(i - 1) == b.charAt(j - 1)) {
					...
					if (max < l[i][j]) {
						...
						if (subStringBeginsAt == lastSubStringBeginsAt) {
							...
							s.append(a.charAt(i - 1));
							// s.append(a.substring(lastSubStringBeginsAt,(i + 1) -
							// lastSubStringBeginsAt));
						}
						max = l[i][j];
					}
				} else {
	...
	public static int minimumCoins(int[] coins, int change) {
		...
		for (int i = 1; i <= change; i++) {
			...
			for (int j = 0; j < coins.length; j++) {
				// Take the coin j value from i and check if there is still some change
				// left
				if (i - coins[j] >= 0) {
	...
	public static int knapsack(int[] v, int[] w, int W, int n) {
		// Store the maximum value possible in dp[i][j] where i = number of items
		// and j = maximum allowed Weight
		...
		for (int i = 0; i <= n; i++) {
			for (int j = 0; j <= W; j++) {
				...
				// Pick the item and if there is still weight left, find the maximum
				// possible value
				if (j - w[i - 1] >= 0) {
	...
	public static boolean isSubset(int[] a, int sum) {
		...
		// Store if a subset exists for s[i][j] where j = sum and i = number of
		// elements in the set
		...
		for (int i = 1; i <= a.length; i++) {
			for (int j = 1; j <= sum; j++) {
				// If value of element is less than required sum then either pick the
				// element and reduce the sum or don't pick the element
				if (j - a[i - 1] >= 0) {
	...
	// Check if 2 subsets exists whose sum (M) is N/2, where N is sum of all
	// elements in array
	...
	// Given an array of positive and negative integers, find an contiguous sub
	...
	public static int findMaxSubArray(int[] a) {
		...
		for (int i = 1; i < a.length; i++) {
			// Check if temp maximum sum is less than 0 then start again from current
			// index and keep track of this index
			if (tempMaxSum < 0) {
				tempMaxSum = a[i];
				tempBeginIndex = i;
			...
			// Check if temp maximum sum is more than maximum sum, if yes update the
			// maximum sum and start,end indexes
	...
	public static void findLongestPalindrome(String s) {
		// Start from the second character till end-1 character and find palindromes
		// of odd and even lengths
		int max = 0, beginIndex = 0;
		for (int i = 1; i < s.length() - 1; i++) {

			// Find the even length palindrome by incrementing the right counter and
			// decrementing the left counter
			int low = i - 1, high = i;
			...
			// Find the odd length palindrome by incrementing the right counter and
			// decrementing the left counter
			low = i - 1;
			high = i + 1;
			while (low >= 0 && high < s.length() && (s.charAt(low) == s.charAt(high))) {
	...
	// // A is empty
	// else if(i==0) {
	// isInterleaved[i][j] = ( isInterleaved[i][j-1] && (b.charAt(j-1) ==
	// c.charAt(j-1)));
	// }
	// // B is empty
	// else if(j==0) {
	// isInterleaved[i][j] = (isInterleaved[i-1][j] && (a.charAt(i-1) ==
	// c.charAt(i-1)));
	// }
	// // Current character of C matches with current character of A,
	// // but doesn't match with current character of B
	// else if((a.charAt(i-1) == c.charAt(i+j-1)) && (b.charAt(j-1) !=
	// c.charAt(i+j-1))) {
	...
	// // Divide the array into 2 subsets such that the difference between them is
	// minimum
	...
	/**
	 * Return the minimum difference between 2 subsets of array. Check if a subset
	 * exists for all possible sums j till Sum and if it exists then take the
	 * difference of sum/2 and j. Check if that difference is less than previously
	 * seen difference and return the most minimum difference.
	 * 
	 * @param a
	 * @return
	 */
